let handler = async (m, { args, text, groupMetadata }) => {
    ...
    const response = await fukusima.groupParticipantsUpdate(m.chat, [user], 'add');
    const jpegThumbnail = await fukusima.profilePictureUrl(m.chat, 'image')
    ...
    for (const participant of response) {
        ...
        if (status === '408') {
            ...
            await fukusima.sendGroupV4Invite(
                m.chat,
                jid,
                inviteCode,
                inviteExp,
