...
let handler = async (m, { text, command, fukusima, usedPrefix }) => {
 if (!text) return m.reply(`Gunakan dengan cara ${usedPrefix + command} *text*`)

 await fukusima.sendMessage(m.chat, {
  react: { text: "⏱️", key: m.key }
 })

 const ephoto = async (url, texk) => {
 ...
 try {
  let hasil = await ephoto(link, text)
  ...
  await m.reply("Gagal membuat efek")
  console.log(err)
 }
}
...
handler.tags = ["maker"]
handler.command = /^(glitchtext|writetext|advancedglow|typographytext|pixelglitch|neonglitch|flagtext|flag3dtext|deletingtext|blackpinkstyle|glowingtext|underwatertext|logomaker|cartoonstyle|papercutstyle|watercolortext|effectclouds|blackpinklogo|gradienttext|summerbeach|luxurygold|multicoloredneon|sandsummer|galaxywallpaper|1917style|makingneon|royaltext|freecreate|galaxystyle|lighteffects)$/i
