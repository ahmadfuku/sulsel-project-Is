...
let handler = async (m, { fukusima, usedPrefix, command, text }) => {
  if (!text) throw `Use ${usedPrefix + command} 12345678`;
  fukusima.sendMessage(m.chat, { react: { text: '🕐', key: m.key }});
...
Member Guild : ${res.result.livemembers}
    ...
    fukusima.reply(m.chat, capt.trim(), m);
  } catch (e) {
    console.error(e);
    fukusima.reply(m.chat, 'Failed to fetch data', m);
  }
};
