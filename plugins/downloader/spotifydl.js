...
let handler = async (m, { fukusima, text, command }) => {
    ...
    let shortLink = text.length > 50 ? text.slice(0, 50) + "..." : text

    // status awal (tampilkan link)
    let proses = await m.reply(
        `⏳ ᴍᴇᴍᴘʀᴏsᴇs ʟɪɴᴋ sᴘᴏᴛɪғʏ...\n\n🔗 ${shortLink}`
    )
...
• ʀɪʟɪs : ${lagu.release_date}`
        ...
        // edit status jadi sukses
        await fukusima.sendMessage(m.chat, {
            text: `✅ ʙᴇʀʜᴀsɪʟ!\n\n🎵 ${lagu.title} — ${lagu.artists}`,
            edit: proses.key
        })
