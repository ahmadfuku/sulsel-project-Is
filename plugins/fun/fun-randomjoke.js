...
let handler = async (m, { fukusima, command }) => {
  if (command === 'randomjoke') {
    ...
    try {
      ...
      await fukusima.sendMessage(m.chat, { text: jokeText }, { quoted: m });
    } catch (err) {
      // Menangani kesalahan jika gagal mengambil lelucon
      console.error(err);
      await fukusima.sendMessage(m.chat, { text: "404 Jokes Not Found", }, { quoted: m }); // Pesan ini akan terkirim jika ada error
    }
  }
};
