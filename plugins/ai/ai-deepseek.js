...
let handler = async (m, { text, usedPrefix, command }) => {
    ...
    if (!fukusima.deepseek) fukusima.deepseek = {};
    if (!fukusima.deepseek[m.sender]) fukusima.deepseek[m.sender] = [];
    fukusima.deepseek[m.sender].push({ role: 'user', content: input });

    try {
        const res = await deepinfra('deepseek-ai/DeepSeek-V3.1', fukusima.deepseek[m.sender]);
        fukusima.deepseek[m.sender].push({ role: 'assistant', content: res });
        m.reply(res)
    } catch (err) {
