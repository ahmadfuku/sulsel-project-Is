...
let handler = async (m, { text, fukusima }) => {
    ...
    while (redirectCount < maxRedirects) {
        ...
        if (/^image\//.test(contentType)) {
            await fukusima.sendFile(m.chat, currentUrl, filename, text, m)
            await res.data?.destroy?.()
            return
        }
        ...
        if (/^application\/json/.test(contentType)) {
            ...
            try {
                ...
                await fukusima.sendFile(m.chat, Buffer.from(pretty), 'file.json', null, m)
                return
            } catch (e) {
                await res.data?.destroy?.()
        ...
        if (/^text\//.test(contentType) || /^text\/html/.test(contentType)) {
            ...
            try {
                ...
                await fukusima.sendFile(m.chat, Buffer.from(txt), contentType === 'text/html' ? 'file.html' : 'file.txt', null, m)
                return
            } catch (e) {
                await res.data?.destroy?.()
        ...
        try {
            ...
            await fukusima.sendFile(m.chat, fileBuf, filename || 'file.bin', text, m)
            return
        } catch (e) {
            await res.data?.destroy?.()
            throw new Error(e.message)
