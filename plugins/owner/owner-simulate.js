let handler = async (m, { fukusima, usedPrefix, command, args: [event], text }) => {
    if (!event) return await fukusima.reply(m.chat, `contoh:
...
${usedPrefix + command} demote @user`.trim(), m)
    ...
    if (act) return fukusima.participantsUpdate({
        id: m.chat,
